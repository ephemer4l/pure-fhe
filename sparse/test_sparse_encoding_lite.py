...
def check_complex_vector_approx_eq(
    vec1,
    vec2,
    error=0.00001,
    ...
    """
    assert len(vec1) == len(vec2), "Length of v1 = %d, Length of v2 = %d" % (
        len(vec1),
        len(vec2),
    )
    for i in range(len(vec1)):
        if (abs(vec1[i].real - vec2[i].real) > error) or (
            abs(vec1[i].imag - vec2[i].imag) > error
        ):
